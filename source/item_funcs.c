...
static int item_get_sell_price_unimplemented(Item* item)
{
    MGBA_FUNC_ERROR("Unimplemented get_sell_price function called for item type type %d", (item)->type);
    return 0;
}
...
static int item_print_desc_unimplemented(Item* item, Rect dest_rect)
{
    MGBA_FUNC_ERROR("Unimplemented print_description function called for item type type %d", (item)->type);
    return 0;
}