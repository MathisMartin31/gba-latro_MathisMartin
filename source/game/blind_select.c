...
static void blind_select_start_anim_seq_reposition_tag_sprites(void);
static void blind_select_handle_input(void);
static void blind_select_handle_immediate_tags_on_init(void);
static void blind_select_handle_immediate_tags_on_update(void);
static void blind_select_reroll_boss_anim_seq_on_init(void);
static void blind_select_reroll_boss_anim_seq_on_update(void);
static void blind_select_reroll_boss_anim_seq_on_exit(void);
static void blind_select_selected_anim_seq_on_init(void);
static void blind_select_selected_anim_seq_on_update(void);
static void blind_select_selected_anim_seq_on_exit(void);
...
static void blind_select_start_anim_seq()
{
    ...
    if (s_timer == TM_SKIP_TAGS_ANIM_SEQ)
    {
        blind_select_start_anim_seq_reposition_tag_sprites();
    }
...
static void blind_select_start_anim_seq_reposition_tag_sprites(void)
{
    ...
    static const BG_POINT tag_sprites_target_positions[NB_SKIPPABLE_BLINDS][NB_SKIPPABLE_BLINDS] = {
    ...
    for (int i = 0; i < NB_SKIPPABLE_BLINDS; i++)
    {
        ...
        {
            sprite_object_set_target(
                (SpriteObject*)blind_skip_tags[i],
...
static inline void blind_select_erase_blind_req_and_reward(enum BlindTokens blind)
{
...
static inline void blind_select_erase_all_blind_reqs_and_rewards(void)
{
    for (enum BlindTokens curr_blind = SMALL_BLIND; curr_blind < NUM_BLINDS_PER_ANTE; curr_blind++)
    {
        blind_select_erase_blind_req_and_reward(curr_blind);
    }
}
...
void increment_blind(enum BlindState increment_reason)
{
    ...
    {
        // defeated small blind: go to big and raise the big blind's skip tag sprite by one tile
...
static void blind_select_selected_anim_seq_on_init(void)
{
    ...
    {
        sprite_object_set_target((SpriteObject*)blind_skip_tags[0], SMALL_BLIND_SKIP_TAG_INIT_POS);
    }
    if (blind_skip_tags[1] != NULL)
    {
        sprite_object_set_target((SpriteObject*)blind_skip_tags[1], BIG_BLIND_SKIP_TAG_INIT_POS);
    }
}
