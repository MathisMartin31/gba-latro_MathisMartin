...
#define BACK_BUTTON_OUTLINE_COLOR_PAL_IDX       11

// Positions/Rects used to construct and update the menu
// Values in number of tiles
// clang-format off
static const Rect     OPTIONS_TILES_SRC_RECT                   = { 0,  0, 15,  5};
...
static GameVariables* game_vars_ptr = NULL;
...
void game_options_menu_change_background(void)
{
    ...
    // Copy tiles to the BG memory and move them out of screen space
    // Takes less space in ROM than having the whole menu already laid out
    ...
    tte_erase_rect_wrapper(OPTIONS_TILES_SRC_RECT);

    // Assemble menu

    // Base window
    main_bg_se_copy_expand_3x3_rect(OPTIONS_BACKGROUND_DEST_RECT, OPTIONS_BACKGROUND_SRC_RECT_3X3_POS);

    // Game Speed button
    // TODO : disable left/right arrows if speed is 1 or 4 (can't go below or above)
    main_bg_se_copy_rect(OPTIONS_SPEED_BTN_SRC_RECT,             OPTIONS_SPEED_BTN_DEST_POS);
    main_bg_se_copy_rect(OPTIONS_SPEED_LESS_ACTIVE_BTN_SRC_RECT, OPTIONS_SPEED_LESS_BTN_DEST_POS);
    main_bg_se_copy_rect(OPTIONS_SPEED_MORE_ACTIVE_BTN_SRC_RECT, OPTIONS_SPEED_MORE_BTN_DEST_POS);

    // High Contrast Cards button
    // TODO : check value and apply "NO" tiles if needed
    main_bg_se_copy_rect(OPTIONS_CONTRAST_BTN_SRC_RECT, OPTIONS_CONTRAST_BTN_DEST_POS);

    // TODO : Music Volume slider

    // TODO : Sound Volume slider
