...
static void game_shop_wait_for_item_desc(void);
static void game_shop_outro(void);

// clang-format off
static StateInfo shop_state_actions[GAME_SHOP_MAX] = {
    [GAME_SHOP_SKIP_TAGS] = STATE_INFO_UPDATE_FN_ONLY(game_shop_redeem_skip_tags),
    ...
    [GAME_SHOP_ITEM_DESC] = STATE_INFO_UPDATE_FN_ONLY(game_shop_wait_for_item_desc),
...
static void game_shop_intro()
{
    ...
    {
        s32 timer_offset = s_timer - 6;

        // TODO: Extract to generic function?
        for (s32 y = 0; y < timer_offset; y++)
        {
...
static void game_shop_process_user_input(void)
{
    ...
    switch (shop_selection_grid.selection.y)
    {
        ...
        case 0:
        {
            ...
            tmp_item = list_get_at_idx(get_jokers_list(), shop_selection_grid.selection.x);
            break;
        }
        ...
        case 1:
        {
            tmp_list = &s_shop_items_list;
            tmp_item = (shop_selection_grid.selection.x > 0)
    ...
    {
        state_machine_change_state(&shop_sm, GAME_SHOP_ITEM_DESC);
